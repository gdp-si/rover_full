...
class EightInchMotorDriver(metaclass=ThreadSafeSingletonMetaclass):
    _driver = None  # NOTE: Manual change in driver will create problem

    def __init__(self):
        EightInchMotorDriver._driver = canalystii.CanalystDevice(
...
class MotorControl:
    ...
    def __init__(self, motor_side: str, config: dict):
        ...
        self.desired_rpm = 0
        self.driver = EightInchMotorDriver()._driver
...
class MotorDrive(metaclass=ThreadSafeSingletonMetaclass):
    """10 inch Motor Drive System Definition for a pair of motors"""

    LOG = Logger("Motor Drive")

    def __init__(self, motor_side="") -> None:
        # print(motor_side)

        assert motor_side in ["left", "right"], "Motor side must be left or right"

        self.left = MotorControl("left", config=MotorDriveConfig.PARAMS_LEFT)
        self.right = MotorControl("right", config=MotorDriveConfig.PARAMS_RIGHT)

        if self.is_active():
            self.LOG.INFO(f" {motor_side} motor is active")

    def is_active(self) -> bool:
        """Check wheather motor is active or not"""
        # print("executed")
        # print(motor_side)
        return self.left.is_active() and self.right.is_active()

    def enable_motor(self):
        self.left.enable_motor()
        self.right.enable_motor()

    # def update (self,L_rotational_rate, R_rotational_rate: float = 0.0):
    #     print(L_rotational_rate,R_rotational_rate)

    #     self.left.update(L_rotational_rate,R_rotational_rate)

    def update(self, rotational_rate: float = 0.0):
        # print(rotational_rate)

        self.left.update(rotational_rate)

        # self.left.update(rotational_rate)
        # self.right.update(rotational_rate)

    def get_odometry(self):
        dist = self.left.lget_wheels_travelled()
        dist1 = self.right.rget_wheels_travelled()
        # dist= int(dist)
        # dist=round(dist)iii
        return dist, dist1
        # print (f_dist)

    def rget_odometry(self):
        dist = self.right.rget_wheels_travelled()
        dist = round(dist)
        # print(type(dist))
        # dist=int(dist)
        return dist
