// Debug Action Server node for PatrolAction.action
...
class PatrolActionDebug : public rclcpp::Node {
 public:
  using Message = roast_interfaces::action::PatrolAction;
  using GoalHandleMessage = rclcpp_action::ServerGoalHandle<Message>;

  explicit PatrolActionDebug(
      const rclcpp::NodeOptions &options = rclcpp::NodeOptions())
      : Node("patrol_action_debug", options) {
    using namespace std::placeholders;
    this->action_server_ = rclcpp_action::create_server<Message>(
        this, "patrol_action",
        std::bind(&PatrolActionDebug::handle_goal, this, _1, _2),
        std::bind(&PatrolActionDebug::handle_cancel, this, _1),
        std::bind(&PatrolActionDebug::handle_accepted, this, _1));
  }
 ...
 private:
  ...
  rclcpp_action::GoalResponse handle_goal(
    ...
    std::string patrol_type =
        goal->patrol_type == Message::Goal::PATROL_SEQUENTIAL ? "sequential"
                                                              : "random";
  ...
  void handle_accepted(const std::shared_ptr<GoalHandleMessage> goal_handle) {
    using namespace std::placeholders;
    std::thread{std::bind(&PatrolActionDebug::execute, this, _1), goal_handle}
        .detach();
  }
  ...
  void execute(const std::shared_ptr<GoalHandleMessage> goal_handle) {
    ...
    for (uint i = 1; i <= 10; i++) {
      ...
      RCLCPP_INFO(this->get_logger(),
                  "Publishing feedback:  Achieved waypoint %d", i);
      loop_rate.sleep();
    }
...
RCLCPP_COMPONENTS_REGISTER_NODE(roast_behavior_tree::PatrolActionDebug)
