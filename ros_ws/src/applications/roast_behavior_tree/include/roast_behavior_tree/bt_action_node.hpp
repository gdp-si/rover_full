...
#ifndef ROAST_BEHAVIOR_TREE__BT_ACTION_NODE_HPP_
#define ROAST_BEHAVIOR_TREE__BT_ACTION_NODE_HPP_
...
class BtActionNode : public BT::ActionNodeBase {
 public:
  /**
   * @brief A roast_behavior_tree::BtActionNode constructor
  ...
  BtActionNode(const std::string &xml_tag_name, const std::string &action_name,
    ...
    if (!config().blackboard->getAny("node")) {
      RCLCPP_WARN(
          rclcpp::get_logger("BtActionNode"),
    ...
    if (!config().blackboard->getAny("bt_loop_duration")) {
      RCLCPP_WARN(node_->get_logger(),
                  ...
                  "default value.");
      bt_loop_duration_ = std::chrono::milliseconds(100);  // 100 ms
    } else {
      bt_loop_duration_ =
    ...
    if (!config().blackboard->getAny("server_timeout")) {
      RCLCPP_WARN(node_->get_logger(),
                  ...
                  "default value.");
      server_timeout_ = std::chrono::milliseconds(3000);  // 3 seconds
    } else {
      server_timeout_ =
          config().blackboard->template get<std::chrono::milliseconds>(
              "server_timeout");
      ...
      if (future_goal_handle_) {
        ...
        if (!is_future_goal_handle_complete(elapsed)) {
          ...
          if (elapsed < server_timeout_) {
            RCLCPP_WARN(
                node_->get_logger(),
                "Waiting for action server to acknowledge goal request for %s",
                action_name_.c_str());
      ...
      if (rclcpp::ok() && !goal_result_available_) {
        ...
        if (goal_updated_ &&
          ...
          if (!is_future_goal_handle_complete(elapsed)) {
            if (elapsed < server_timeout_) {
              RCLCPP_WARN(node_->get_logger(),
                          "Waiting for action server to acknowledge goal "
                          "request for %s",
                          action_name_.c_str());
              return BT::NodeStatus::RUNNING;
            }
...
#endif  // ROAST_BEHAVIOR_TREE__BT_ACTION_NODE_HPP_
