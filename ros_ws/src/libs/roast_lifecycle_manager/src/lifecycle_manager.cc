#include "roast_lifecycle_manager/lifecycle_manager.hpp"
...
namespace roast_lifecycle_manager {

LifecycleManager::LifecycleManager(const rclcpp::NodeOptions &options)
    : Node("roast_lifecycle_manager", options), diagnostics_updater_(this) {
  ...
  declare_parameter("use_bond_communication", rclcpp::ParameterValue(false));
  declare_parameter("no_of_retries", 5);

  registerRclPreshutdownCallback();

  node_names_ = get_parameter("node_names").as_string_array();
  no_of_retries = get_parameter("no_of_retries").as_int();
  get_parameter("autostart", autostart_);
  double bond_timeout_s;
  get_parameter("use_bond_communication", use_bond_communication_);
  ...
  bond_respawn_max_duration_ =
      rclcpp::Duration::from_seconds(respawn_timeout_s);

  get_parameter("attempt_respawn_reconnection", attempt_respawn_reconnection_);

  callback_group_ = create_callback_group(
      rclcpp::CallbackGroupType::MutuallyExclusive, false);
  manager_srv_ = this->create_service<nav2_msgs::srv::ManageLifecycleNodes>(
  ...
  diagnostics_updater_.setHardwareID("Roast");
  diagnostics_updater_.add("Roast Health", this,
                           &LifecycleManager::CreateActiveDiagnostic);
}
...
void LifecycleManager::CreateActiveDiagnostic(
    diagnostic_updater::DiagnosticStatusWrapper &stat) {
  if (system_active_) {
    stat.summary(diagnostic_msgs::msg::DiagnosticStatus::OK, "Roast is active");
  } else {
...
bool LifecycleManager::createBondConnection(const std::string &node_name) {
  const double timeout_ns =
      std::chrono::duration_cast<std::chrono::nanoseconds>(bond_timeout_)
          .count();
  ...
  if (bond_map_.find(node_name) == bond_map_.end() &&
    ...
    RCLCPP_INFO(get_logger(), "Server %s connected with bond.",
                node_name.c_str());
  }
...
bool LifecycleManager::changeStateForNode(const std::string &node_name,
  ...
  if (!node_map_[node_name]->change_state(transition) ||
    ...
    RCLCPP_ERROR(get_logger(), "Failed to change state for node: %s",
                 node_name.c_str());
    return false;
  }

  if (transition == Transition::TRANSITION_ACTIVATE) {
    if (use_bond_communication_) {
...
bool LifecycleManager::changeStateForAllNodes(std::uint8_t transition,
                                              bool hard_change) {
  bool success = true;
  ...
  if (transition == Transition::TRANSITION_CONFIGURE ||
    ...
    for (auto &node_name : node_names_) {
      int i = 0;
      while (i < no_of_retries)  // try multiple times before giving up
      {
        try {
          if (changeStateForNode(node_name, transition)) {
            break;  // exit the loop if the state change was successful
          } else if (!hard_change) {
            RCLCPP_ERROR(get_logger(),
                         "Failed to change state for node: %s trying %d time",
                         node_name.c_str(), i);
          }
        } catch (const std::runtime_error &e) {
          RCLCPP_ERROR(get_logger(),
                       "Failed to change state for node: %s. Exception: %s.",
      ...
      if (i >=
          no_of_retries)  // check if we reached the maximum number of retries
      {
        success = false;
      }
    }
...
void LifecycleManager::createBondTimer() {
  if (bond_timeout_.count() <= 0 && !use_bond_communication_) {
    return;
  }
...
void LifecycleManager::onRclPreshutdown() {
  RCLCPP_INFO(get_logger(),
              "Running Roast LifecycleManager rcl preshutdown (%s)",
              this->get_name());
  if (use_bond_communication_) {
    destroyBondTimer();
    bond_map_.clear();
...
void LifecycleManager::checkBondConnections() {
  ...
  for (auto &node_name : node_names_) {
    ...
    if (bond_map_[node_name]->isBroken()) {
      ...
      if (attempt_respawn_reconnection_ && use_bond_communication_) {
        bond_respawn_timer_ = this->create_wall_timer(
            1s, std::bind(&LifecycleManager::checkBondRespawnConnection, this),
            callback_group_);
      }
      return;
    }
...
void LifecycleManager::checkBondRespawnConnection() {
  ...
  for (auto &node_name : node_names_) {
    ...
    try {
      node_map_[node_name]
          ->get_state();  // Only won't throw if the server exists
      live_servers++;
    } catch (...) {
      break;
    }
  ...
  if (live_servers == max_live_servers) {
    ...
    message(
        "Failed to re-establish connection from a server crash after maximum "
        "timeout.");
    bond_respawn_start_time_ = rclcpp::Time(0);
    bond_respawn_timer_.reset();
  }
...
void LifecycleManager::message(const std::string &msg) {
  RCLCPP_INFO(get_logger(), ANSI_COLOR_BLUE "\33[1m%s\33[0m" ANSI_COLOR_RESET,
              msg.c_str());
}
...
RCLCPP_COMPONENTS_REGISTER_NODE(roast_lifecycle_manager::LifecycleManager)
