...
#ifndef ROAST_LIFECYCLE_MANAGER__LIFECYCLE_MANAGER_CLIENT_HPP_
#define ROAST_LIFECYCLE_MANAGER__LIFECYCLE_MANAGER_CLIENT_HPP_
...
class LifecycleManagerClient {
 ...
 protected:
   ...
   * @param command
   */
  bool callService(uint8_t command, const std::chrono::nanoseconds timeout =
                                        std::chrono::nanoseconds(-1));

  // The node to use for the service call
  rclcpp::Node::SharedPtr node_;

  std::shared_ptr<nav2_util::ServiceClient<ManageLifecycleNodes>>
      manager_client_;
  std::shared_ptr<nav2_util::ServiceClient<std_srvs::srv::Trigger>>
      is_active_client_;
...
#endif  // ROAST_LIFECYCLE_MANAGER__LIFECYCLE_MANAGER_CLIENT_HPP_
