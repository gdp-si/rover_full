...
LoggerUsage::LoggerUsage(rclcpp::NodeOptions options)
  ...
  pub_ = create_publisher<std_msgs::msg::String>("roast_logging_count", 10);
  timer_ = create_wall_timer(500ms, std::bind(&LoggerUsage::on_timer, this));
  debug_function_to_evaluate_ =
      std::bind(is_divisor_of_twelve, std::cref(count_), get_logger());
...
void LoggerUsage::on_timer() {
  ...
  RCLCPP_DEBUG_EXPRESSION(get_logger(), (count_ % 2) == 0,
...
RCLCPP_COMPONENTS_REGISTER_NODE(roast_logging::LoggerUsage)
