...
#include "roast_slam/string_tools.hpp"
...
class PclToMap : public rclcpp::Node {
 public:
  ...
  PclToMap() : Node("pcl_to_map") {
    ...
    t_sub_ = create_subscription<geometry_msgs::msg::Vector3>(
        "~/translate", sub_qos,
        std::bind(&PclToMap::t_cb, this, std::placeholders::_1));
    r_sub_ = create_subscription<geometry_msgs::msg::Vector3>(
        "~/rotate", sub_qos,
        std::bind(&PclToMap::r_cb, this, std::placeholders::_1));

    auto pub_qos = rclcpp::QoS(1);
    pub_qos.reliable();
    pub_qos.transient_local();
    cloud_pub_ =
        create_publisher<sensor_msgs::msg::PointCloud2>("~/cloud", pub_qos);
    og_pub_ = create_publisher<nav_msgs::msg::OccupancyGrid>("~/map", pub_qos);

    map_save_server_ = create_service<std_srvs::srv::Trigger>(
        "~/save", std::bind(&PclToMap::save_svc, this, std::placeholders::_1,
                            std::placeholders::_2, std::placeholders::_3));

    og_msg_ptr_ = new nav_msgs::msg::OccupancyGrid();
    og_msg_ptr_->header.frame_id = "map";
    og_msg_ptr_->info.resolution =
        declare_parameter<double>("resolution", 0.05);

    z_filter_ = new PassThrough<PointXYZ>();
    z_filter_->setFilterFieldName("z");
    z_filter_->setFilterLimits(declare_parameter<double>("zmin", 0.0),
                               declare_parameter<double>("zmax", 1.8));
    voxelgrid_filter_ = new VoxelGrid<PointXYZ>();
    voxelgrid_filter_->setLeafSize(og_msg_ptr_->info.resolution,
                                   og_msg_ptr_->info.resolution, FLT_MAX);
    voxelgrid_filter_->setMinimumPointsNumberPerVoxel(
 ...
 private:
  ...
  void save_svc(const std::shared_ptr<rmw_request_id_t> req_header,
    ...
    // write 2D map image
    cv::Mat om_mat = cv::Mat(
                cv::Size(og_msg_ptr_->info.width, og_msg_ptr_->info.height),
                CV_8U, &og_msg_ptr_->data[0]),
            om_mat_flip = cv::Mat(om_mat.size(), CV_8U),
            occ_mask = cv::Mat(om_mat.size(), CV_8U);
    cv::flip(om_mat, om_mat_flip, 0);
    cv::inRange(om_mat_flip, OCCUPIED_CELL_VALUE, OCCUPIED_CELL_VALUE,
                occ_mask);
  ...
  bool load_cloud() {
    ...
    const auto prob_thresh =
        declare_parameter<double>("occupancy_likelihood_threshold", 0.5);
    if (fp != "") {
      ...
      if (cloud_in_ext_ == ".pcd") {
        ...
        RCLCPP_ERROR(get_logger(),
                     ("Unrecognized extension: " + cloud_in_ext_).c_str());
      }

      if (cloud_loaded()) {
        RCLCPP_INFO(get_logger(),
                    ("Loaded " + fp + " (" +
                     std::to_string(cloud_ptr_->size()) + " points)")
                        .c_str());
      } else {
  ...
  void update(const Affine3d &transform) {
    ...
    feature_extractor.setInputCloud(cloud_downsample);
    feature_extractor.compute();
    PointXYZ min_point_AABB, max_point_AABB;
    feature_extractor.getAABB(min_point_AABB, max_point_AABB);

    og_msg_ptr_->info.width = uint32_t(std::ceil(
        (max_point_AABB.x - min_point_AABB.x) / og_msg_ptr_->info.resolution));
    og_msg_ptr_->info.height = uint32_t(std::ceil(
        (max_point_AABB.y - min_point_AABB.y) / og_msg_ptr_->info.resolution));
    og_msg_ptr_->data.resize(og_msg_ptr_->info.height *
                             og_msg_ptr_->info.width);
    ...
    og_msg_ptr_->data.resize(og_msg_ptr_->info.width *
                             og_msg_ptr_->info.height);
    std::fill(og_msg_ptr_->data.begin(), og_msg_ptr_->data.end(),
              FREE_CELL_VALUE);
    for (const auto &pt : *cloud_downsample) {
      uint32_t r = std::round((pt.x - og_msg_ptr_->info.origin.position.x) /
                              og_msg_ptr_->info.resolution),
               c = std::round((pt.y - og_msg_ptr_->info.origin.position.y) /
                              og_msg_ptr_->info.resolution);
  ...
  Affine3d transform_from_param() {
    ...
    const auto initial_transform =
        declare_parameter<std::string>("transform", "");
    if (initial_transform != "") {
      ...
      switch (doubles.size()) {
        ...
        case 7:
          transform.translate(Vector3d(doubles[0], doubles[1], doubles[2]));
          transform.rotate(
              Quaterniond(doubles[6], doubles[3], doubles[4], doubles[5]));
          break;
        default:
          RCLCPP_ERROR(
              get_logger(),
              ("Invalid transform parameter: " + initial_transform).c_str());
          break;
      }
    }
...
int main(int argc, char *argv[]) {
  ...
  if (ok) {
    rclcpp::spin(ptm->get_node_base_interface());
  }
